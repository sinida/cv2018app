...
int main(int argc, const char** argv)
{
	...
	cv::Mat frame = cv::imread("Imagedata/facetest.jpg", 1);
	...
	if (!frame.empty()){
		...
		Facedetect faceDet;
		faceDet.Init();
		faceDet.Detect(frame, faceXY_que);
		//faceDet.UnInit();

		vector<cv::Rect> eyeXY_que;
		Eyedetect eyeDet;
		eyeDet.Init();
		eyeDet.Detect(frame, eyeXY_que);
		//eyeDet.UnInit();
		face_draw(faceXY_que, frame);
		eye_draw(eyeXY_que, frame);
