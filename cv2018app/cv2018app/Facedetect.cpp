...
int Facedetect::Init() {
	//cout << "Face Init" << endl;
	string face_cascade_name = "model/opencv/haarcascade_frontalface_alt.xml";
...
int Facedetect::Detect(cv::Mat in_img, vector<cv::Rect> &out_Vec) {
	...
	//-- Detect faces
	face_cascade.detectMultiScale(frame_gray, out_Vec, 1.1, 2, 0 | CV_HAAR_SCALE_IMAGE, cv::Size(30, 30));
